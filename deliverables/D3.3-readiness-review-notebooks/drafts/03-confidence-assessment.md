...
| D3.4 — IFRC user demonstration                  | Not started                                                               | **Medium**                                | Last in the dependency chain; depends on all three of the above, but has runway to December                                                                                                         |
...
static-site compilation: the platform's riskiest assumption, that a parameterized notebook can be run and
...
design and build work. Confidence is lowest for D3.4 landing in a fully realized,
...
- **D3.2** to high: D3.3 reaching a state where its notebook/environment contract is fixed enough to
  document without near-term rework.
- **D3.4** to high: a concrete demonstration date set, once D3.1–D3.3 are far enough along to support
  one.
